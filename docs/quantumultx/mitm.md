...
![MITM1](Photo/MITM1.webp){: width=1500}

- 进入「系统设置」 → 「已下载描述文件」 → 输入密码并安装

![MITM2](Photo/MITM2.webp){: width=900}

- 进入「系统设置」 → 「通用」 → 「关于本机」 → 最下方「证书信任设置」 → 勾选刚才安装的证书

![MITM3](Photo/MITM3.webp){: width=900}
...
<!-- prettier-ignore -->
!!! 警告 "重要"
     请务必保管好你的 CA 密码短语（passphrase）和 p12 私钥，避免泄露。
...
- 可使用前缀 `-` 将特定主机名排除；
- 配置文件中，多个主机名可用 `,` 分割；
- 单独的 `*` 符号作为主机名参数会匹配所有主机名；
- 仅匹配的主机名的 TLS SNI 或目标地址会被 MitM 处理；

#### 6.3.1 UI 查看 & 添加

点击右下角风车进入「设置」 → 「MitM」区域-「主机名」

![HOSTNAME](Photo/HOSTNAME.webp){: width=900}
...
# 当使用 Quantumult X 在 M 芯片的 Mac 设备上作为局域网网关时，使用下面的参数来跳过某些特定源 IP 的 MitM 需求
# 支持通配符 * 和 ?
;skip_src_ip = 192.168.4.50, 92.168.4.51

# 当多个不同的 TCP 连接（非域名类请求）的目标 IP 不同，但这些连接的 TLS 握手 SNI 字段相同时，
# 如需跳过其中某些连接的 MitM hostname 匹配过程，可使用此参数。支持通配符 * 和 ?
...
- `skip_validating_cert`：是否跳过验证证书，默认 `false`
- `skip_src_ip`：TCP 连接的源 IP 匹配此列表时，跳过主机名评估。支持通配符 `*` 和 `?`
- `skip_dst_ip`：TCP 连接的目标 IP 匹配此列表时，跳过主机名评估。支持通配符 `*` 和 `?`
