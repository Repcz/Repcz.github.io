...
如果 iOS 版本小于 iOS 13.0，最大的 `doh-server` 数量将被限制为 1；如果 iOS 版本小于 iOS 15.0，最大的 `doh-server` 数量将被限制为 2。

当使用的 `doh-server` 不是基于 HTTP/2 时，DoH 将被暂时禁用，普通的服务器将被使用，直到下次启动 VPN 连接。

当同时设置 `doq-server` 和 `doh-server` 时，查询将同时发送到所有服务器。
...
prefer-doh3
```
当 `prefer-doh3` 被设置时，DoH 将尝试通过 HTTP/3 (DNS over HTTP3) 的查询，如果失败，查询连接将退回到 HTTP/2。

由于 HTTP/2 和 HTTP/3 连接都支持复用，当改变该设置时它可能不会立即生效，但可以通过重新连接 Quantumult X 隧道或手动改变当前网络环境，让它立即生效。

#### 5.6.3 自定义 DoH 的 User Agent

DoH 的 `User Agent` 需要在 `[general]` 设置：见 [通用设置 - 自定义 DoH 的 User Agent](general.md)
...
该 SSID 排除/包含机制也适用于 `doh-server` 和 `doq-server`：

```
[dns]
; doq-server 在 SSID1 下排除
;doq-server = quic://dns.adguard.com, excluded_ssids=SSID1

; doq-server 仅在 SSID2 下生效
;doq-server = quic://dns.adguard.com, included_ssids=SSID2

; doh-server 在 SSID1 下排除
;doh-server = https://223.6.6.6/dns-query, excluded_ssids=SSID1

; doh-server 仅在 SSID2 下生效
