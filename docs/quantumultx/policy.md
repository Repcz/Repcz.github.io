...
Policy：除了内置的 [3 个策略](filter.md?#31-filter)，还有以下策略：
...
![Static_1](Photo/Static_1.webp){: width=600}
...
`img-url` 自定义图标参数是可选的，详见[节点资源图标](node.md?#25)

##### UI添加

当然你也可以通过UI创建这样一个策略组

![Static_2](Photo/Static_2.webp){: width=900}
...
![Available](Photo/Available.webp){: width=600}
...
![Round-Robin](Photo/Round-Robin.webp){: width=600}
...
![Dest-Hash](Photo/Dest-Hash.webp){: width=600}
...
![URL-Latency-Benchmark](Photo/URL-Latency-Benchmark.webp){: width=600}
...
![SSID](Photo/SSID.webp){: width=900}
...
<!-- prettier-ignore -->
!!! 注意
    `server-tag-regex=` 参数无法筛选 `[server_local]` 下的本地节点，因此推荐本地节点以「配置片段」的方式添加
...
前文所述每条分流规则必须有[对应的策略](filter.md?#31-filter)。
...
修改策略组`参数`
  ...
  ![GroupFIX](Photo/GroupFIX.webp){: width=900}
